  ...
  consumeQueue = async (
    ...
    await channel.consume(
      ...
      async (message: MessageBroker.GenericType) => {
        ...
        if (
          ...
          logger.log(
            `Limit of consumed messages has been reached:  Queue[${consumeQueueOptions.queueName}]Prefetch[${prefetch}]Count[${messages.length}]`
          );
          processMessages();
        }
      },
    ...
    setInterval(async () => {
      if (
        ...
        {
          logger.log(
            `Limit of consumed messages not reached: Queue[${consumeQueueOptions.queueName}]Prefetch[${prefetch}]Count[${messages.length}]`
          );
          processMessages();
        }
    }, 5000);
