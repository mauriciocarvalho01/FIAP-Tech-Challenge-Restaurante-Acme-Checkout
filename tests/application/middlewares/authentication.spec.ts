...
describe('AuthenticationMiddleware', () => {
  ...
  describe('validateAuthorization', () => {
    it('should return true when authorization is valid', async () => {
      const authorization = 'valid-token';
      const ip = '192.168.1.1'
      mockAuthorization.token = authorization;
      tokenHandler.authorization.mockReturnValue(mockAuthorization);
      validator.validate.mockResolvedValue([]); // Retorna lista vazia indicando sucesso

      const result = await sut['validateAuthorization']({ authorization });

      expect(result).toBe(true);
    });

    it('should return false when authorization is invalid', async () => {
      const authorization = 'invalid-token';
      const ip = '192.168.1.1'
      mockAuthorization.token = authorization;
      tokenHandler.authorization.mockReturnValue(mockAuthorization);
      validator.validate.mockResolvedValue(['error']); // Retorna erro
